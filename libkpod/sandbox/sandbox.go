...
// NetNs contains information pertaining to the network namespace for the sandbox
type NetNs struct {
	sync.Mutex
	ns       ns.NetNS
	symlink  *os.File
...
// NetNsGet Gets the NetNs associated with the given nspath and name
...
var (
	// ErrIDEmpty is the error returned when the PodSandboxId is empty
	ErrIDEmpty = errors.New("PodSandboxId should not be empty")
	// ErrClosedNetNs is the error returned when the PodSandbox networking namespace is closed
